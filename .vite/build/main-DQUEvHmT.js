...
const { dirname: dirname$2 } = require$$1;
...
const { dirname: dirname$1 } = require$$1;
...
var electron_1 = __importDefault(require$$0);
var fs_1 = __importDefault(require$$0$2);
var mkdirp_1 = __importDefault(mkdirp_1$1);
var path_1 = __importDefault(require$$1);
...
const libraryPath = () => {
  ...
  if (!_library || typeof _library !== "string") {
    settings$2.setSync(
      "library",
      process.env.LIBRARY_PATH || require$$1.join(require$$0.app.getPath("documents"), LIBRARY_PATH_SUFFIX)
    );
  } else if (require$$1.parse(_library).base !== LIBRARY_PATH_SUFFIX) {
    settings$2.setSync("library", require$$1.join(_library, LIBRARY_PATH_SUFFIX));
  }
...
const cachePath = () => {
  const tmpDir = require$$1.join(libraryPath(), "cache");
  fs.ensureDirSync(tmpDir);
  return tmpDir;
};
const settings = {
  registerIpcHandlers: () => {
    require$$0.ipcMain.handle("settings-get", (_event, key) => {
      return settings$2.getSync(key);
    });
    require$$0.ipcMain.handle("settings-set", (_event, key, value) => {
      settings$2.setSync(key, value);
    });
    require$$0.ipcMain.handle("settings-get-library", (_event) => {
      libraryPath();
      return settings$2.getSync("library");
    });
    require$$0.ipcMain.handle("settings-set-library", (_event, library) => {
      if (require$$1.parse(library).base === LIBRARY_PATH_SUFFIX) {
        settings$2.setSync("library", library);
      } else {
        const dir = require$$1.join(library, LIBRARY_PATH_SUFFIX);
...
require$$0.app.whenReady().then(() => {
  if (!require$$0.app.isPackaged) {
    Promise.resolve().then(() => require("./index-HfW4kINY.js")).then((n) => n.index).then((mymodule) => {
  ...
  require$$0.protocol.handle(SUFFIX, (request) => {
    ...
    if (url.match(/library\/(audios|videos|recordings|speeches|segments)/g)) {
      ...
      url = require$$1.join(settings.libraryPath(), url);
    } else if (url.startsWith("library")) {
      url = url.replace("library/", "");
      url = require$$1.join(settings.libraryPath(), url);
    }
    return require$$0.net.fetch(`file:///${url}`);
  });
  createWindow();
});
